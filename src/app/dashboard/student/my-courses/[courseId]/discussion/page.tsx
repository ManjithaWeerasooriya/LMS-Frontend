...
import {
  getCourseDiscussion,
  postCourseDiscussionMessage,
  type CourseDiscussionMessage,
} from '@/lib/student';
...
export default function CourseDiscussionPage() {
  ...
  const loadDiscussion = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getCourseDiscussion(courseId);
      setThreads(data);
    } catch {
      setError('Unable to load discussion. You may not be enrolled.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePostNew = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(null);
      await postCourseDiscussionMessage(courseId, newMessage.trim());
      setNewMessage('');
  ...
  const handleReplySubmit = async (
    ...
    try {
      ...
      setError(null);
      await postCourseDiscussionMessage(courseId, replyContent.trim(), parentId);
      setReplyContent('');
      setReplyForId(null);
      await loadDiscussion();
    } catch {
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-2">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Chat with other students enrolled in this course.
          </p>
        </div>
      </header>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form onSubmit={handlePostNew} className="mt-3 space-y-3">
          <textarea
            ...
            rows={3}
            placeholder="Share a question, idea, or update with your classmates..."
            className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none ring-0 transition placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100"
          />
          <div className="flex items-center justify-between gap-3">
            <p className="text-[11px] text-slate-400">
              Only students enrolled in this course can view and reply.
            </p>
            <button
              type="submit"
      ...
      {loading ? (
        ...
        <p className="py-8 text-center text-sm text-slate-500">
          No messages yet. Start the first conversation for this course.
        </p>
      ) : (
